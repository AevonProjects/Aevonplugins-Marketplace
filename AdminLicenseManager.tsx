...
export default function AdminLicenseManager() {
  ...
  async function performAction(
    ...
    try {
      ...
      const body = action === "reset-server"
        ? { resetActivation: true }
        : { status: action === "activate" ? "active" : action === "suspend" ? "suspended" : "revoked" };
      ...
      const result = await response.json().catch(() => null);
      if (!response.ok) throw new Error(result?.error || result?.message || "Unable to update this license.");

      const messages = {
        activate: "License activated successfully.",
        suspend: "License suspended successfully.",
        revoke: "License revoked successfully.",
        "reset-server": "Server activation reset successfully. The license can now be activated on another server.",
      };
      setMessage(messages[action]);
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to update this license.");
    }
  }
