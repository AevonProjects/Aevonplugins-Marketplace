...
async function decorate(admin:any, rows:any[]){
  ...
  const counts=new Map<string,number>(); for(const r of replies||[])counts.set(r.thread_id,(counts.get(r.thread_id)||0)+1);
  return rows.map(r=>({...r,author:pm.get(r.user_id)||null,reply_count:counts.get(r.id)||0}));
}
...
export async function GET(request:Request){
  ...
  let wallet=null; let viewer:any={id:null,role:null};
  const token=request.headers.get('authorization')?.replace(/^Bearer\s+/i,'');
  if(token){const {data:u}=await admin.auth.getUser(token);if(u.user){await admin.from('aevonsmp_forum_wallets').upsert({user_id:u.user.id},{onConflict:'user_id',ignoreDuplicates:true});const {data:w}=await admin.from('aevonsmp_forum_wallets').select('*').eq('user_id',u.user.id).maybeSingle();wallet=w;const {data:p}=await admin.from('profiles').select('role').eq('id',u.user.id).maybeSingle();viewer={id:u.user.id,role:p?.role||null};}}
  return NextResponse.json({threads:await decorate(admin,data||[]),wallet,viewer});
}
