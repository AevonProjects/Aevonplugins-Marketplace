...
export async function POST(request:Request){
 ...
 const orderCode=`ASMP-GCASH-${Date.now().toString(36).toUpperCase()}-${randomBytes(3).toString("hex").toUpperCase()}`; const amount=Number(p.price)*quantity;
 const {data,error}=await auth.admin.from("aevonsmp_orders").insert({order_code:orderCode,user_id:auth.user.id,customer_email:auth.user.email||"unknown",product_id:p.id,product_name:p.name,unit_price:Number(p.price),quantity,amount,currency:"PHP",minecraft_ign:ign,reward_command:p.reward_command,command_mode:p.command_mode,required_free_slots:p.required_free_slots,payment_method:"gcash",payment_status:"pending",delivery_status:"awaiting_payment"}).select("id,order_code,amount,payment_status,delivery_status").single();
