...
export async function POST(request:Request){
 ...
 if(error||!data)return NextResponse.json({error:error?.message||"Could not create GCash order."},{status:500});
 try{
   const ticketId=await ensureGcashTicket(auth.admin,{orderKind:"aevonsmp",orderId:data.id,orderCode:data.order_code,userId:auth.user.id,customerEmail:auth.user.email||"unknown",productName:p.name,amount:Number(data.amount)});
   return NextResponse.json({order:data,ticketId});
 }catch(e:any){return NextResponse.json({error:e?.message||"Order created but ticket creation failed."},{status:500})}
}
